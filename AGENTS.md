...
- Current scope: plugin shell plus native HID input, auto-reconnect, aircraft/tailnum-based profile selection, key-image upload, worker-thread key polling, flight-loop event dispatch, prefs, structured logging with debug mode, profile parsing, and command dispatch.
